...
describe("Vesting Test", function () {
    ...
    beforeEach(async function () {
        ...
        await rech.setAntisnipeDisable();
        await rech.setLiquidityRestrictorDisable();

        const VestingFactory = await ethers.getContractFactory("Vesting");
        vesting = await VestingFactory.deploy(rech.address);
    ...
    describe("Hold", function () {
        ...
        it("Can't hold with incorrect start and end", async function () {
            await rech.approve(vesting.address, 100000);
        ...
        it("Can hold correct", async function () {
            await rech.approve(vesting.address, 300000);
            ...
            expect(await rech.balanceOf(adr1.address)).to.equal(0);
            expect(await rech.balanceOf(adr2.address)).to.equal(0);
            expect(await rech.balanceOf(adr3.address)).to.equal(0);
    ...
    describe("Claim", function () {
        ...
        it("Claiming correct share before vesting end", async function () {
            ...
            expect(await rech.balanceOf(adr1.address)).to.equal(
                Math.floor(
                    10000 +
        ...
        it("Claiming full share after vesting end", async function () {
            ...
            await vesting.claim(adr1.address);
            expect(await rech.balanceOf(adr1.address)).to.equal(100000);
        });
        ...
        it("Can claim unlocked if lastUpdate now", async function () {
            ...
            expect(await rech.balanceOf(adr1.address)).to.equal(10000);
